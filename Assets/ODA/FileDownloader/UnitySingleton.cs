...
namespace Scripts._Libs.Downloader
{
    /// <summary>
    ...
    public abstract class UnitySingleton<T> : MonoBehaviour where T : MonoBehaviour
    {
        ...
        public static T Instance
        {
            ...
            set
            {
                ...
                if (attribute == null)
                    UnityEngine.Debug.LogError("Cannot find UnitySingleton attribute on " + typeof(T).Name);

                if (attribute.allowSetInstance)
                    instance = value;
                else
                    UnityEngine.Debug.LogError(typeof(T).Name + " is not allowed to set instances.  Please set the allowSetInstace flag to true to enable this feature.");
            }
        }
        ...
        private static bool TryGenerateInstance(UnitySingletonAttribute.Type type, bool destroyOnLoad, string resourcesLoadPath, bool warn)
        {
            if (type == UnitySingletonAttribute.Type.ExistsInScene)
            {
                ...
                {
                    if (warn)
                        UnityEngine.Debug.LogError("Cannot find an object with a " + typeof(T).Name + " .  Please add one to the scene.");
                    return false;
                }
            }
            else if (type == UnitySingletonAttribute.Type.LoadedFromResources)
            {
                ...
                {
                    if (warn)
                        UnityEngine.Debug.LogError("UnitySingletonAttribute.resourcesLoadPath is not a valid Resources location in " + typeof(T).Name);
                    return false;
                }
                ...
                {
                    if (warn)
                        UnityEngine.Debug.LogError("Failed to load prefab with " + typeof(T).Name + " component attached to it from folder Resources/" + resourcesLoadPath + ".  Please add a prefab with the component to that location, or update the location.");
                    return false;
                }
                ...
                {
                    if (warn)
                        UnityEngine.Debug.LogError("Failed to create instance of prefab " + pref + " with component " + typeof(T).Name + ".  Please check your memory constraints");
                    return false;
                }
            }
            else if (type == UnitySingletonAttribute.Type.CreateOnNewGameObject)
            {
                ...
                {
                    if (warn)
                        UnityEngine.Debug.LogError("Failed to create gameobject for instance of " + typeof(T).Name + ".  Please check your memory constraints.");
                    return false;
                }
                ...
                {
                    if (warn)
                        UnityEngine.Debug.LogError("Failed to add component of " + typeof(T).Name + "to new gameobject.  Please check your memory constraints.");
                    Destroy(go);
                    return false;
                }
            }
